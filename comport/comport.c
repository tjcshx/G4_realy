...
static int init_uart_agcs(int argc, char **argv, comport_t *comport);
...
int open_uart(comport_t *comport)
{
    ...
    {
        log_info("%s:[%d] is not a Terminal equipment.\n",comport->device_name, comport->fd);
        return -1;
    }
...
int send_data(comport_t *comport)
{
    if (NULL == comport)
    {
        log_error("Illegal input arguments.\n");
    ...
    rv = write(comport->fd, comport->send_data, comport->len);
    if (rv == comport->len)
    {
